...
if(localStorage.getItem('todo')){
    ...
    document.querySelectorAll("[id=second_page]").forEach(el => el.style.display = "inline");
...
addTask.addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
        ...
        if (document.getElementById("first_page").style.display === "inline" | document.getElementById("third_page").style.display === "inline"){
          document.querySelectorAll("[id=first_page]").forEach(el => el.style.display = "none");
...
function displayTasks(){
    ...
    for(const [index, task] of todoList.entries()){
        ...
        <button class="task-deleter"><svg class='deleter-svg' id='item_${index}' width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path class='deleter-svg' fill-rule="evenodd" clip-rule="evenodd" d="M7.41401 6L11.707 1.707C12.098 1.316 12.098 0.684 11.707 0.293C11.316 -0.0979999 10.684 -0.0979999 10.293 0.293L6.00001 4.586L1.70701 0.293C1.31601 -0.0979999 0.684006 -0.0979999 0.293006 0.293C-0.0979941 0.684 -0.0979941 1.316 0.293006 1.707L4.58601 6L0.293006 10.293C-0.0979941 10.684 -0.0979941 11.316 0.293006 11.707C0.488006 11.902 0.744006 12 1.00001 12C1.25601 12 1.51201 11.902 1.70701 11.707L6.00001 7.414L10.293 11.707C10.488 11.902 10.744 12 11 12C11.256 12 11.512 11.902 11.707 11.707C12.098 11.316 12.098 10.684 11.707 10.293L7.41401 6Z" fill="#252526"/>
          </svg></button>
      </div>`;
      todo.innerHTML = displayTask;
    }
    taskToday.innerHTML = `${todoList.length}  tasks to go`;
}
...
todo.addEventListener('click', function(event){
    ...
    if (event.target.classList[0] === "task-deleter" | event.target.classList[0] === "deleter-svg"){
